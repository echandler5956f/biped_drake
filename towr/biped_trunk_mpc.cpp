...
#include "../lcm_types/trunklcm/biped_trunk_state_t.hpp"
...
void publish_trunk_state(SplineHolder solution, double t, bool finished=false) {
    ...
    lcm::LCM lcm;
    trunklcm::biped_trunk_state_t state;
    ...
    // Foot positions
    Eigen::Map<Eigen::VectorXd>(&state.l_p[0], 3) = solution.ee_motion_.at(L)->GetPoint(t).p();
    Eigen::Map<Eigen::VectorXd>(&state.r_p[0], 3) = solution.ee_motion_.at(R)->GetPoint(t).p();

    // Foot velocities
    Eigen::Map<Eigen::VectorXd>(&state.l_pd[0], 3) = solution.ee_motion_.at(L)->GetPoint(t).v();
    Eigen::Map<Eigen::VectorXd>(&state.r_pd[0], 3) = solution.ee_motion_.at(R)->GetPoint(t).v();

    // Foot accelerations
    Eigen::Map<Eigen::VectorXd>(&state.l_pdd[0], 3) = solution.ee_motion_.at(L)->GetPoint(t).a();
    Eigen::Map<Eigen::VectorXd>(&state.r_pdd[0], 3) = solution.ee_motion_.at(R)->GetPoint(t).a();

    // Foot contact states
    state.l_contact = solution.phase_durations_.at(L)->IsContactPhase(t);
    state.r_contact = solution.phase_durations_.at(R)->IsContactPhase(t);

    // Foot contact forces
    Eigen::Map<Eigen::VectorXd>(&state.l_f[0], 3) = solution.ee_force_.at(L)->GetPoint(t).p();
    Eigen::Map<Eigen::VectorXd>(&state.r_f[0], 3) = solution.ee_force_.at(R)->GetPoint(t).p();

    lcm.publish("trunk_state", &state);
}

// Generate a trunk-model trajectory for a biped using TOWR, and send the results
...
int main(int argc, char *argv[]) {
    ...
    // desired goal state
    formulation.final_base_.lin.at(towr::kPos) << dist_x, dist_y, 1.1318-0.1;
