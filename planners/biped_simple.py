...
class BipedBasicTrunkPlanner(LeafSystem):
    """
    ...
    def __init__(self, frame_ids):
        ...
        # Dictionary of geometry frame ids {"trunk": trunk_frame_id, "l": l_foot_frame_id, ...}
    ...
    def SimpleStanding(self):
        ...
        """
        # Foot positions
        # self.output_dict["p_l"] = np.array([ 0.0640419246633535+0.05, 0.0827964816855771+0.1, 0.0])   # huron
        # self.output_dict["p_r"] = np.array([ 0.0640419246633535+0.05,-0.0827964816855771-0.1, 0.0])
        self.output_dict["p_l"] = np.array([ 0.0, 0.0827964816855771+0.1, 0.0])   # huron
        self.output_dict["p_r"] = np.array([ 0.0,-0.0827964816855771-0.1, 0.0])

        # Foot velocities
        self.output_dict["pd_l"] = np.zeros(3)
        self.output_dict["pd_r"] = np.zeros(3)
        
        # Foot accelerations
        self.output_dict["pdd_l"] = np.zeros(3)
        self.output_dict["pdd_r"] = np.zeros(3)
        ...
        self.output_dict["f_cj"] = np.zeros((3,2))

        # Body pose
        self.output_dict["rpy_body"] = np.array([0.0, 0.0, 0.0])
        self.output_dict["p_body"] = np.array([0.0, 0.0, 1.1318-0.025])
