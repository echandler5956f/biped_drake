...
from planners import BipedBasicTrunkPlanner, BipedTowrTrunkPlanner
import os
import sys
...
biped = Parser(plant=plant).AddModelFromFile(robot_urdf,"biped")
...
for foot in ["l","r"]:
    ...
    foot_shape = Box(0.221, 0.078, 0.002)
    # foot_shape = Sphere(0.05)
...
if planning_method == "basic":
    planner = builder.AddSystem(BipedBasicTrunkPlanner(trunk_frame_ids))
elif planning_method == "towr":
    planner = builder.AddSystem(BipedTowrTrunkPlanner(trunk_frame_ids))
else:
    print("Invalid planning method %s" % planning_method)
    sys.exit(1)

if control_method == "B":
    controller = builder.AddSystem(BipedBasicController(plant,dt,use_lcm=use_lcm))
elif control_method == "ID":
    controller = builder.AddSystem(BipedIDController(plant,dt,use_lcm=use_lcm))
elif control_method == "MPTC":
    controller = builder.AddSystem(BipedMPTCController(plant,dt,use_lcm=use_lcm))
elif control_method == "PC":
    controller = builder.AddSystem(BipedPCController(plant,dt,use_lcm=use_lcm))
elif control_method == "CLF":
    controller = builder.AddSystem(BipedCLFController(plant,dt,use_lcm=use_lcm))
else:
...
q0 = np.asarray([1.0, 0.0, 0.0, 0.0,    # base orientation
                 0.0, 0.0, 1.1318-0.025,  # base position
                 0.0, 0.1158, -0.0906, 0.3925, -0.3019, -0.1158, 
                 0.0, -0.1158, -0.0906, 0.3925, -0.3019, 0.1158])
# q0 = np.asarray([1.0, 0.0, 0.0, 0.0,    # base orientation
#             0.0, 0.0, 1.1318,  # base position
